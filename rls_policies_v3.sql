...
DROP POLICY IF EXISTS "Authenticated users can insert souvenirs" ON public.souvenirs;
DROP POLICY IF EXISTS "Authenticated users can update their own souvenirs" ON public.souvenirs;
DROP POLICY IF EXISTS "Authenticated users can delete their own souvenirs" ON public.souvenirs;
DROP POLICY IF EXISTS "Souvenirs are visible to everyone" ON public.souvenirs;

DROP POLICY IF EXISTS "Authenticated users can insert souvenir_locations" ON public.souvenir_locations;
DROP POLICY IF EXISTS "Authenticated users can update their own souvenir_locations" ON public.souvenir_locations;
DROP POLICY IF EXISTS "Authenticated users can delete their own souvenir_locations" ON public.souvenir_locations;
...
CREATE POLICY "Authenticated users can insert souvenirs" ON public.souvenirs FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Authenticated users can insert souvenir_locations" ON public.souvenir_locations FOR INSERT WITH CHECK (auth.uid() IN (SELECT user_id FROM souvenirs WHERE id = souvenir_id));
